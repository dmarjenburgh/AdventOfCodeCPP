...
int main() {
    ...
    const int totalScore{std::accumulate(pairs.cbegin(), pairs.cend(), 0, [](auto acc, auto p) { return acc + Score(p); })};
    const int totalScore2{std::accumulate(pairs.cbegin(), pairs.cend(), 0, [](auto acc, auto p) { return acc + Score({p[0], FindMove(p)}); })};
