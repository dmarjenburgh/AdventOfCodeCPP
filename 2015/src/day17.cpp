...
int numsubsets(const FI from, const FI to, int target, const int size, std::vector<int>& sols) {
    if (target == 0) {
        sols.push_back(size);
        return 1;
    }
...
int main() {
    ...
    std::vector<int> sols;
    const int num_combinations{numsubsets(sizes.begin(), sizes.end(), TARGET, 0, sols)};
