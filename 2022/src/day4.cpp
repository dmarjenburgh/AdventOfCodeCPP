...
int main() {
    ...
    size_t numContained = ranges::count_if(pairs, [](auto rp) {return isContained(rp[0], rp[1]) || isContained(rp[1], rp[0]);});
    size_t numOverlaps = ranges::count_if(pairs, [](auto rp) {return overlaps(rp[0], rp[1]);});
