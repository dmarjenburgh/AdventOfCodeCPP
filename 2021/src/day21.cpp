...
/**
 * Returns the number of ways the player at a given position will reach the goal within a given number of turns
 * and the number of ways the player does not reach the goal within that number of turns.
 */
std::pair<uint64_t, uint64_t> count_possibilities(int position, int points_left, int turns_left) {
  if (!turns_left) return points_left <= 0 ? std::make_pair(1, 0) : std::make_pair(0, 1);
  if (points_left <= 0) return {1, 0};

  std::pair<uint64_t, uint64_t> total_cnt{};
  for (auto [v, n] : distribution) {
    const int new_pos{(position + v) % 10};
    const int score{new_pos + 1};
    const auto cnt{count_possibilities(new_pos, points_left - score, turns_left - 1)};
...
int main() {
  ...
  std::transform(range.cbegin(), range.cend(), branches1.begin(),
                 [&p1](int n) { return count_possibilities(p1, SCORE_GOAL, n); });
  std::transform(range.cbegin(), range.cend(), branches2.begin(),
                 [&p2](int n) { return count_possibilities(p2, SCORE_GOAL, n); });

  uint64_t p1_wins{}, p2_wins{};
  for (const auto t : range) {
    if (t > 0) {
      // Ways in which p1 reaches >= 21 in t turns and p2 doesn't in < t turns
      p1_wins += (branches1[t].first - branches1[t - 1].first) * branches2[t - 1].second;
      // Ways in which p2 reaches >= 21 in t turns and p1 doesn't in <= t turns
      p2_wins += (branches2[t].first - branches2[t - 1].first) * branches1[t].second;
    }
  }
