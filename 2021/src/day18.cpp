...
/**
 * Returns the indices of the opening and closing bracket of the next exploding pair
 * or (npos, npos) if not found.
 */
...
std::pair<int, int> pair_numbers(const std::string& pair_string) {
  std::pair<int, int> p;
  int x{};
  std::for_each(pair_string.cbegin() + 1, pair_string.cend() - 1, [&p, &x](const char& ch) {
    if (ch == ',') {
      p.first = x;
      x = 0;
    } else {
...
void add_number(SnailFishNumber& s, int x, size_t from, bool reverse) {
  ...
  size_t i{from};
  while (i and i < s.size() and !std::isdigit(s[i])) i += dir;

  if (!i or i == s.size()) return;

  size_t first_pos = i;

  i = static_cast<int>(first_pos);
  while (std::isdigit(s[i])) i += dir;
...
bool split(SnailFishNumber& s) {
  ...
  for (auto it = s.begin() + 1; it != send; it++) {
    ...
    if (prev and cur) {
      ...
      int l{v / 2}, r{v - l};
      s.replace(it - 1, it + 1, "[" + std::to_string(l) + "," + std::to_string(r) + "]");
      return true;
    }
    prev = cur;
  }
...
size_t find_comma(const SnailFishNumber& s) {
  ...
  for(const char ch: s) {
    ...
    else if (depth == 1 and ch == ',') return i;
    i++;
  }
...
size_t magnitude(const SnailFishNumber& s) {
  if (std::isdigit(s[0]))
    ...
    return 3 * magnitude(s.substr(1, comma_pos - 1)) + 2 * magnitude(s.substr(comma_pos + 1));
  }
}
...
size_t largest_magnitude_sum(const std::vector<SnailFishNumber>& v) {
  ...
  for (int i{0}; i < v.size(); ++i) {
    for (int j{i + 1}; j < v.size(); ++j) {
      if (size_t m{magnitude(add(v[i], v[j]))}; m > max_mag) max_mag = m;
      if (size_t m{magnitude(add(v[j], v[i]))}; m > max_mag) max_mag = m;
    }
  }
  return max_mag;
}
...
int main() {
  ...
  while (std::getline(file, line)) v.emplace_back(std::move(line));
