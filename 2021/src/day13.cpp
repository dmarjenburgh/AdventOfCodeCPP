...
struct FoldInstruction {
  ...
  FoldDirection dir;
  int distance;
  FoldInstruction(FoldDirection dir, int distance)
...
class Paper {
  ...
  void doFold(const FoldInstruction &fi) {
    ...
    for (const auto &dot : dots) {
      if (fi.dir == FoldInstruction::FoldDirection::horizontal and
          dot.y > fi.distance) {
        Dot new_dot{dot.x, 2 * fi.distance - dot.y};
        newdots.insert(new_dot);
      } else if (fi.dir == FoldInstruction::FoldDirection::vertical and
                 dot.x > fi.distance) {
...
std::istream &operator>>(std::istream &is, Paper &paper) {
  ...
  while (std::getline(is, line)) {
    ...
    if (std::regex_search(line, sm, r)) {
      FoldInstruction::FoldDirection dir =
          (sm[1] == 'x') ? FoldInstruction::FoldDirection::vertical
...
std::ostream &operator<<(std::ostream &os, const Paper &paper) {
  const auto width{std::max_element(
      paper.dots.cbegin(), paper.dots.cend(),
      [](const Dot &d1, const Dot &d2) { return d1.x < d2.x; })};
  const auto height{std::max_element(
      paper.dots.cbegin(), paper.dots.cend(),
...
int main() {
  ...
  paper.startFolding(); // Does first fold again, but folds are idempotent
                        // anyway.
  std::cout << "Part 2:\n" << paper << '\n';
}
