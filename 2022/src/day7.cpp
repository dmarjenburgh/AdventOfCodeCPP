...
int du(Dir &dir) {
    ...
    int total_dir_size = std::accumulate(dir.dirs.begin(), dir.dirs.end(), 0, [](int acc, auto &child) { return acc + du(child); });
    int total_file_size = std::accumulate(dir.files.begin(), dir.files.end(), 0, [](int acc, auto &child) { return acc + child.size; });
    dir.totalSize = total_dir_size + total_file_size;
    return dir.totalSize;
}

int sum_of_dirs(const Dir &dir, const int threshold) {
    int subdirs{std::accumulate(dir.dirs.begin(), dir.dirs.end(), 0, [&threshold](int acc, auto child) { return acc + sum_of_dirs(child, threshold); })};
...
int main() {
    ...
    for (auto it{lines.begin() + 1}; it != lines.end(); ++it) {
        ...
        if (cmd == "cd") {
            ...
            if (auto result{ranges::find_if(currentDir->dirs, [&new_dir](Dir &d) { return d.name == new_dir; })}; result != currentDir->dirs.end()) {
                currentDir = &(*result);
            } else if (new_dir == "..") {
            ...
            while ((it + 1) != lines.end() && !(it + 1)->starts_with('$')) {
                ...
                if (l.starts_with("dir")) {
                    ...
                    currentDir->files.emplace_back(File{.name = l.substr(e + 1), .size = filesize});
                }
            }
        }
    }
