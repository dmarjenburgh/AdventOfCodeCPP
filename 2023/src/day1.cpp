...
int find_digit(std::string_view s) {
    ...
    if (m == digits.end()) { return -1; }
    int i = std::distance(digits.begin(), m);
    return (i > 8) ? i - 8 : i + 1;
}

int calibrationValue(const std::string &s) {
    auto first{*std::find_if(s.begin(), s.end(), [](char c) { return std::isdigit(c); })};
    auto last{*std::find_if(s.rbegin(), s.rend(), [](char c) { return std::isdigit(c); })};
...
int realCalibrationValue(const std::string_view s) {
    ...
    for (auto it{s.begin()}; it != s.end(); ++it) {
        ...
        if (digit >= 0) { found_digits.push_back(digit); };
    }
    return found_digits.front() * 10 + found_digits.back();
};

int sumValues(const std::vector<std::string> &lines, auto f) {
    return std::accumulate(lines.begin(), lines.end(), 0, [&f](int acc, auto &l) { return acc + f(l); });
}
