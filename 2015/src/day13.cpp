...
int calculate_happiness(const std::vector<std::string> &names, std::unordered_map<std::string, int> &lookup) {
    int total{};
    for (int i{}; i < names.size() - 1; ++i) {
        total += lookup[names[i] + names[i + 1]] + lookup[names[i + 1] + names[i]];
    }
    return total + lookup[names[names.size() - 1] + names[0]] + lookup[names[0] + names[names.size() - 1]];
}
