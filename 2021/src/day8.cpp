...
std::ostream &operator<<(std::ostream &os, const Pattern &pattern) {
  for (auto &s : pattern.patternlist)
    os << s << ' ';
  os << '|';
  for (auto &s : pattern.output)
    os << ' ' << s;
  os << '\n';
  return os;
}
...
void set_remaining_mapping(const PatternList &l, Mapping &m, int len, char ch) {
  const auto sp = std::find_if(
      l.cbegin(), l.cend(), [len](const auto &s) { return s.length() == len; });
  const auto c = std::find_if(sp->cbegin(), sp->cend(),
                              [&m](char c) { return !m.contains(c); });
  m[*c] = ch;
}
...
// Find the pattern with 2 segments, that is the 1. The unmapped segment maps to
// 'c' Find the pattern with 3 segments, that is the 7 and the new segment maps
// to 'a' Find the pattern with 4 segments, that is the 4. The unmapped segment
// maps to 'd' Find the pattern with 7 segments, that is the 8. The unmapped
// segments maps to 'g'
...
int main() {
  ...
  int sum =
      std::accumulate(patterns.cbegin(), patterns.cend(), 0,
