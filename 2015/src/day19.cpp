...
int num_matches(const std::string &molecule,
                const std::multimap<std::string, std::string> &rmap) {
  ...
  for (auto it = start; it != end; ++it) {
    ...
    for (auto i = range.first; i != range.second; ++i) {
      auto s = molecule.substr(0, it->position()) + i->second +
               molecule.substr(it->position() + it->length());
      products.insert(s);
    }
  }
...
std::string reduce(const std::string &molecule,
                   const std::multimap<std::string, std::string> &rmap) {
  for (const auto &kv : rmap) {
    const size_t found = molecule.find(kv.second);
    if (found != std::string::npos) {
      std::ostringstream oss;
      oss << molecule.substr(0, found) << kv.first
          << molecule.substr(found + kv.second.length());
      return oss.str();
    }
  }
  return std::string();
