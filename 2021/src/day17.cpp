...
bool hits_target_area(const Region &r, int vx, int vy) {
  ...
  for (;;) {
    if (point.first > r.xrange.second)
      return false;
    if (point.second < r.yrange.first && vy < 0)
      return false;
...
std::tuple<Range, int> viable_range(const Region &r) {
  ...
  // After vy reaches 0, the y coordinates are identical to the y-values in the
  // upward phase In particular, it will always reach 0 again. vy must be
  // smaller than the distance of the furthest part of the y-range.
...
std::tuple<int, int> aim(const Region &region) {
  // Scan the x-range, starting with the max vy and going down until we hit the
  // target area
...
int main() {
  ...
  // Assert that the y-range does not contain 0. You could reach infinite height
  // if it does.
  assert(region.yrange.first * region.yrange.second > 0);
  // We also require the x-range to exclude 0. Assuming that to be an invalid
  // puzzle input :P
  assert(region.xrange.second < 0 or region.xrange.first > 0);

  // Problem is symmetric wrt y-axis. Use this to normalize the target area to
  // the x > 0 region.
